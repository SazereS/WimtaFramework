...
%s
    ...
    public function __construct($message){
        ...
        $this->_limit = (Settings::getInstance()->debug_stacktrace_limit) ? Settings::getInstance()->debug_stack_limit : 10;
        $this->_stack = debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT, $this->_limit);
    }

    public function build(){
        if(Settings::getInstance()->debug_stacktrace_style == 'simple'){
            $this->_formatted = '<h3>' . $this->_message . '</h3><pre>' . print_r($this->_stack, true) . '</pre>';
        } else {
